...
export default function ShowBookingsPage({
  ...
  const { action, bookingId } = useMemo(() => {
  ...
  const existsBookingUpdatable = new Date(bookings[0].checkin) > new Date() && bookings[0].status.includes('CF');
  ...
  const goToReserva = (appointmentId: string, bookingId: string) => {
    ...
    router.push(
      `/dashboard/user-multiownership-appointment/${appointmentId}/bookings/${bookingId}/edit${from === "admin" ? "?from=admin" : ""}`
    );
  };
            ...
            <Grid xs={12}>
              ...
              >
                ...
                {existsBookingUpdatable && (
                  ...
                  >
                    <Button
                      onClick={() => goToReserva(appointmentId, booking.id.toString())}
                      sx={{
